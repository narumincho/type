 ...
 * 型
 */
export type Type = { _: "Int" } | { _: "String" } | { _: "Bool" } | { _: "List"; type_: Type } | { _: "Maybe"; type_: Type } | { _: "Result"; resultType: ResultType } | { _: "Id"; string_: string } | { _: "Token"; string_: string } | { _: "Custom"; string_: string };
 ...
 *
 */
export const typeToken = (string_: string): Type => ({ _: "Token", string_: string_ });
...
export const encodeInt32 = (value: number): ReadonlyArray<number> => {
  ...
  while (true) {
    ...
    value >>= 7;
    if (value === 0 && (byte & 64) === 0 || value === -1 && (byte & 4) !== 0) {
      result.push(byte);
      return result;
    }
...
export const encodeString = (text: string): ReadonlyArray<number> => {
  const result: ReadonlyArray<number> = Array["from"](new (process === undefined || process.title === "browser"?TextEncoder:a.TextEncoder)().encode(text));
 ...
 *
 */
export const encodeMaybe = <T>(encodeFunction: (a: T) => ReadonlyArray<number>): (a: Maybe<T>) => ReadonlyArray<number> => (maybe: Maybe<T>): ReadonlyArray<number> => {
  switch (maybe._) {
    case "Just": {
 ...
 *
 */
export const encodeResultType = (resultType: ResultType): ReadonlyArray<number> => (encodeType(resultType.ok).concat(encodeType(resultType.error)));
 ...
 *
 */
export const decodeInt = (index: number, binary: Uint8Array): { result: number; nextIndex: number } => {
  let result: number = 0;
  let offset: number = 0;
  while (true) {
...
export const decodeString = (index: number, binary: Uint8Array): { result: string; nextIndex: number } => {
  const length: { result: number; nextIndex: number } = decodeInt(index, binary);
  const nextIndex: number = index + length.nextIndex + length.result;
  const textBinary: Uint8Array = binary.slice(index + length.nextIndex, nextIndex);
  const isBrowser: boolean = process === undefined || process.title === "browser";
  if (isBrowser) {
    return { result: new TextDecoder().decode(textBinary), nextIndex: nextIndex };
  }
  return { result: new a.TextDecoder().decode(textBinary), nextIndex: nextIndex };
};
...
export const decodeMaybe = <T>(decodeFunction: (a: number, b: Uint8Array) => { result: T; nextIndex: number }): (a: number, b: Uint8Array) => { result: Maybe<T>; nextIndex: number } => (index: number, binary: Uint8Array): { result: Maybe<T>; nextIndex: number } => {
  const patternIndexAndNextIndex: { result: number; nextIndex: number } = decodeInt(index, binary);
 ...
 *
 */
export const decodeType = (index: number, binary: Uint8Array): { result: Type; nextIndex: number } => {
  const patternIndex: { result: number; nextIndex: number } = decodeInt(index, binary);
  ...
  if (patternIndex.result === 3) {
    const result: { result: Type; nextIndex: number } = decodeType(patternIndex.nextIndex, binary);
  ...
  if (patternIndex.result === 5) {
    const result: { result: ResultType; nextIndex: number } = decodeResultType(patternIndex.nextIndex, binary);
    return { result: typeResult(result.result), nextIndex: result.nextIndex };
  }
  if (patternIndex.result === 6) {
    const result: { result: string; nextIndex: number } = decodeString(patternIndex.nextIndex, binary);
 ...
 *
 */
export const decodeResultType = (index: number, binary: Uint8Array): { result: ResultType; nextIndex: number } => {
  const okAndNextIndex: { result: Type; nextIndex: number } = decodeType(index, binary);
  const errorAndNextIndex: { result: Type; nextIndex: number } = decodeType(okAndNextIndex.nextIndex, binary);
  return { result: { ok: okAndNextIndex.result, error: errorAndNextIndex.result }, nextIndex: errorAndNextIndex.nextIndex };
};
 ...
 *
 */
export const decodeLanguage = (index: number, binary: Uint8Array): { result: Language; nextIndex: number } => {
  const patternIndex: { result: number; nextIndex: number } = decodeInt(index, binary);
  if (patternIndex.result === 0) {
